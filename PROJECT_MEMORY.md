...
As of 2026-06-02, the user approved Version 2 to add exactly one new feature: **AI 建议** during card creation. Adjust create-page field order so users fill **语境句子 → 生词 → 释义** top-to-bottom; after sentence and word are entered, AI may ghost-suggest a one-word/short Chinese contextual meaning in the 释义 field. User can accept with Enter, delete with Backspace, or overwrite. Also add a separate AI suggestion/note explaining how the word is used in that context; user can keep, modify, or delete it. If no AI suggestion exists, display `none`. Settings must support OpenAI-compatible generic AI API configs with display name, Base URL, API Key, and model; support switching among multiple configs with one active config and no hard product-level max count. API Keys are stored locally, masked in UI, and excluded from exports; users re-enter keys after import. Do not broaden V2 into other AI/dictionary/future features unless the user explicitly approves.
