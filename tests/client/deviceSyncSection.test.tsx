...
it('shows both transports and persists explicit LAN selection', async () => {
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
    ...
    if (url === '/api/device-sync/tailscale') return Promise.resolve(json({ installed: true, online: true, dns_name: 'pc.tailnet.ts.net', configured_url: null, serve_command: 'tailscale serve --bg 3108', serve_available: true }));
    if (url === '/api/device-sync/lan' && init?.method === 'PATCH') {
      lanPatch = JSON.parse(init.body as string);
      return Promise.resolve(json({ enabled: true, restart_required: true }));
    }
    return Promise.resolve(json({}));
  });

  render(<I18nProvider><DeviceSyncSection /></I18nProvider>);
  expect(await screen.findByText('局域网')).toBeInTheDocument();
  ...
  expect(await screen.findByText(/请重启 PC 应用/u)).toBeInTheDocument();
});
