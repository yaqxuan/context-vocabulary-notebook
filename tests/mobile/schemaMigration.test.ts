...
describe('alpha.5 Android database migration', () => {
  it('preserves pairing and outbox rows while adding language state', () => {
    ...
    db.exec(`
      ...
      CREATE TABLE fsrs_states (card_id TEXT PRIMARY KEY, last_reviewed_at TEXT, same_day_retry_at TEXT);
      ...
      INSERT INTO fsrs_states VALUES ('known', '2026-07-18T10:00:00.000Z', '2026-07-18T10:00:00.000Z');
    `);
    ...
    for (const column of MOBILE_V4_CONFIG_COLUMNS) db.exec(`ALTER TABLE mobile_config ADD COLUMN ${column.definition}`);
    db.exec("UPDATE mobile_config SET connection_mode = 'tailscale', last_successful_transport = 'tailscale'");
    db.exec(MOBILE_V4_BACKFILL_AGAIN_COOLDOWN);
    db.prepare('INSERT INTO mobile_schema_migrations VALUES (?, ?)').run(MOBILE_SCHEMA_MIGRATION_VERSION, new Date().toISOString());

    expect(db.prepare(`
      SELECT server_id, credential, snapshot_revision, interface_language,
        connection_mode, last_successful_transport FROM mobile_config
    `).get()).toEqual({
      server_id: 'pc-1', credential: 'credential-1', snapshot_revision: 26,
      interface_language: '中文', connection_mode: 'tailscale', last_successful_transport: 'tailscale',
    });
    ...
    expect(db.prepare('SELECT MAX(version) FROM mobile_schema_migrations').pluck().get()).toBe(4);
    expect(db.prepare('SELECT same_day_retry_at FROM fsrs_states WHERE card_id = ?').get('known'))
      .toEqual({ same_day_retry_at: '2026-07-18T10:10:00.000Z' });
    db.close();
  });
});
