...
ローカルファーストの軽量な文脈語彙復習ツール。外国語の動画を見る際に、未知の単語、元の文、ローカルの動画クリップ、スクリーンショット、音声、タグを記録し、FSRSアルゴリズムを使って復習をスケジュールするのに適しています。
...
- 1つの意味エントリに複数の文脈インスタンスを関連付けることができ、異なる動画での同じ意味の用法を記録するのに最適です。
- ローカルメディアの添付：動画 `mp4`、音声 `mp3`、画像 `jpg / png / webp`。
- FSRS間隔反復法：`Again` / `Good` の2つの評価ボタンのみを保持。
- 意味エントリのリスト、検索、タグによるフィルタリング、お気に入り、統計。
- ZIPインポート・エクスポート：完全な個人バックアップと純粋なカード共有をサポート。
- V2カード作成ページのAI提案：OpenAI互換のAPIを設定して、文脈上の定義や用法のメモを提案できます。APIキーはローカルにのみ保存されます。
