...
> プラットフォーム状況：Windows/WSL は実機確認済み、Linux は CI 確認済みです。macOS インストーラーは実装済みですが実機 Mac では未検証のため、現時点では実験的です。Android の署名済み APK は [GitHub Releases](https://github.com/yaqxuan/context-vocabulary-notebook/releases) で提供しています。iOS / iPadOS は `v0.3.x` では提供しません。
