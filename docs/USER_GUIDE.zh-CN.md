...
每种导出都可以选择“全部语言”或一种指定的学习语言。全部语言的个人备份还包含
全局设置；按语言导出的个人备份保留该语言的复习状态，但不包含全局设置。

导入时先扫描 ZIP，再选择文件中一种或多种语言。恢复“全部语言个人备份”中的所有
语言时，会同时恢复全局设置并撤销旧的手机配对；只导入指定语言则不会改变全局
设置、其他语言词库或当前手机配对。
